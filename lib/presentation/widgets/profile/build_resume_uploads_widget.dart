...
class _ResumeCardState extends State<ResumeCard> {
  ...
  @override
  void didUpdateWidget(ResumeCard oldWidget) {
  ...
  @override
  void dispose() {
    // Safely close PDF document without waiting
    _pdfDocument?.close().catchError((error) {
  ...
  void _initializePdf() async {
    ...
    if (widget.certificate is File) {
      ...
      Logger.log(" ===> Certificate is a URL string --> ${widget.title}");
      String url = widget.certificate as String;
      Logger.log(" ===> URL: $url --> ${widget.title}");
      if (_isPdfUrl(url)) {
        Logger.log(" ===> Detected as PDF URL --> ${widget.title}");
  ...
  Widget _generatePreviewWidget() {
    ...
    if (widget.certificate is String) {
      Logger.log(
        " ===> Generating preview for URL: ${widget.certificate} --> ${widget.title}",
      );
      String url = widget.certificate as String;
  ...
  Widget _buildPdfFromUrl(String url) {
    Logger.log("Building PDF preview from URL: $url --> ${widget.title}");
    // If download error occurred, show error
    if (_downloadError != null) {
      Logger.log("Download error: $_downloadError --> ${widget.title}");
      return _buildErrorPlaceholder(_downloadError!);
    }

    // If currently downloading, show loading
    if (_isDownloading) {
      Logger.log(" ===> Downloading PDF... --> ${widget.title}");
      return _buildLoadingPlaceholder(
    ...
    if (_pdfDocument != null) {
      Logger.log(
        " ===> PDF loaded, showing preview -> $_pdfDocument --> ${widget.title}",
      );
      return _buildPdfPreviewFromDocument(_pdfDocument!);
    }
    ...
    if (_pdfLoadError) {
      Logger.log(" ===> PDF load error --> ${widget.title}");
      return _buildPdfPlaceholder(
        showFileName: true,
    ...
    if (_cachedPdfPath != null) {
      Logger.log(
        " ===> Loading cached PDF from $_cachedPdfPath --> ${widget.title}",
      );
      return _buildLoadingPlaceholder(
        AppLocalizations.of(context)!.processingPdf,
      );
    ...
    Logger.log(" ===> Starting download for PDF --> ${widget.title}");
  ...
  Future<void> _downloadPdfAsync(String url) async {
    if (_isDownloading || _cachedPdfPath != null) {
      ...
      Logger.log(
        " ===> Already downloading or cached, skipping download --> ${widget.title}",
      );
      return;
    }
    ...
    try {
      ...
      final fileName =
          'temp_cert_cv_${DateTime.now().millisecondsSinceEpoch}.pdf';
  ...
  Widget _buildPdfPreviewFromDocument(PdfDocument document) {
    if (document.pagesCount == 0) {
      return _buildErrorPlaceholder(AppLocalizations.of(context)!.emptyPdfFile);
    }
    Logger.log(" ===> Rendering PDF preview $document --> ${widget.title}");
    return Container(
      height: 120,
      width: double.infinity,
      color: Colors.grey.shade100,
